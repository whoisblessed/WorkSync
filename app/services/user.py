...
class UserService:
    ...
    async def get_by_id(self, id: int) -> User:
        db_user = await self.user_repository.get_active_by_id(id)
        if db_user is None:
            raise NotFoundException(f"Пользователь с ID {id} не найден или неактивен")

        return db_user

    async def register(self, user: UserCreate, current_user: User) -> User:
    ...
    async def update(self, user: UserUpdate, current_user: User) -> User:
        db_user = await self.user_repository.get_active_by_id(current_user.id)
        if db_user is None:
